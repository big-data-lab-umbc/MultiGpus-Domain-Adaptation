...
def cuda_load_data(args):
    folder_src = os.path.join(args.data_dir, args.src_domain)
    folder_tgt = os.path.join(args.data_dir, args.tgt_domain)
    source_loader, n_class = data_loader.cuda_load_data(
        hvd, folder_src, args.batch_size, infinite_data_loader=not args.epoch_based_training, train=True, num_workers=args.num_workers)
    target_train_loader, _ = data_loader.cuda_load_data(
        hvd, folder_tgt, args.batch_size, infinite_data_loader=not args.epoch_based_training, train=True, num_workers=args.num_workers)
    target_test_loader, _ = data_loader.cuda_load_data(
        hvd, folder_tgt, args.batch_size, infinite_data_loader=False, train=False, num_workers=args.num_workers)
    return source_loader, target_train_loader, target_test_loader, n_class
...
def get_optimizer(model, args):
    initial_lr = args.lr*args.lr_scaler if not args.lr_scheduler else 1.0*args.lr_scaler
...
def main():
    ...
    if args.device == "cuda":
        ...
        source_loader, target_train_loader, target_test_loader, n_class = cuda_load_data(args)
    else:
    ...
    lr_scaler = hvd.size() if not args.use_adasum else 1.0
    ...
    setattr(args, "lr_scaler", lr_scaler)
